...
func (a *Advertisement) Start() error {
	...
	if a.adapter.connectHandler != nil {
		...
		a.adapter.bus.Signal(property)
		propertiesChangedMatchOption := dbus.WithMatchInterface("org.freedesktop.DBus.ObjectManager")
		a.adapter.bus.AddMatchSignal(propertiesChangedMatchOption)

		go func() {
			for sig := range property {
				if sig.Name != "org.freedesktop.DBus.ObjectManager.InterfacesAdded" && sig.Name != "org.freedesktop.DBus.ObjectManager.InterfacesRemoved" {
					continue
				}

				interfaceName := string(sig.Body[0].(dbus.ObjectPath))
				if !strings.HasPrefix(interfaceName, "/org/bluez/hci0/dev_") || strings.Contains(interfaceName, "/service") {
					continue
				}

				body := sig.Body[1].(map[string]map[string]dbus.Variant)

				macString := body["org.bluez.Device1"]["Address"]
				mac, _ := ParseMAC(macString.String())
				addr := Address{MACAddress: MACAddress{MAC: mac}}

				isConnected := sig.Name == "org.freedesktop.DBus.ObjectManager.InterfacesAdded"
				a.adapter.connectHandler(Device{Address: addr}, isConnected)
			}
		}()
	}
